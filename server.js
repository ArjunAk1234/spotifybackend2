...
const connectDB = async () => {
  if (mongoose.connection.readyState >= 1) {
    return; // ✅ If already connected, reuse the connection
  }
  try {
    await mongoose.connect(uri1, {
      useNewUrlParser: true,
      useUnifiedTopology: true,
      serverSelectionTimeoutMS: 5000, // Prevent long connection timeouts
    });
...
connectDB(); // ✅ Ensure MongoDB connects before handling requests
...
const MusicSchema = new mongoose.Schema({
  ...
  artist: String,
  genre: String,
  file: { type: Buffer, select: false }, // ✅ Exclude binary data from default queries
  albumCover: { type: Buffer, select: false } // ✅ Exclude album cover from default queries
}, { collection: 'music' });
...
app.post('/upload-song', upload.fields([{ name: 'file', maxCount: 1 }, { name: 'albumCover', maxCount: 1 }]), async (req, res) => {
  ...
  try {
    const newSong = new Music({
      ...
      albumCover: req.files['albumCover'] ? req.files['albumCover'][0].buffer : null 
    });
    ...
    res.status(201).json({
      ...
      song: {
        ...
        artist: newSong.artist,
        genre: newSong.genre,
        fileUrl: `/song/${newSong._id}`,
        albumCoverUrl: newSong.albumCover ? `/album-cover/${newSong._id}` : null
      }
    });
  } catch (err) {
    console.error('Error uploading song:', err);
...
// ✅ Get All Songs API (Returns URLs, Not Buffers)
app.get('/songs', async (req, res) => {
  try {
    const songs = await Music.find({}, '_id title artist genre');
    res.status(200).json(songs.map(song => ({
      ...song.toObject(),
      fileUrl: `/song/${song._id}`,
      albumCoverUrl: song.albumCover ? `/album-cover/${song._id}` : null
    })));
  } catch (err) {
    console.error('Error fetching songs:', err);
    res.status(500).json({ message: 'Server error' });
...
app.get('/song/:id', async (req, res) => {
  try {
    const song = await Music.findById(req.params.id).select('file');
    if (!song || !song.file) {
    ...
    console.error('Error fetching song file:', err);
    res.status(500).json({ message: 'Server error' });
  }
});
...
app.get('/album-cover/:id', async (req, res) => {
  try {
    const song = await Music.findById(req.params.id).select('albumCover');
    if (!song || !song.albumCover) {
      return res.status(404).json({ message: 'Album cover not found' });
    }

    res.set('Content-Type', 'image/jpeg');
    res.send(song.albumCover);
  } catch (err) {
    console.error('Error fetching album cover:', err);
    res.status(500).json({ message: 'Server error' });
  }
...
// ✅ Export for Vercel (No `app.listen(3000)`)
module.exports = app;
